...
SharedTests::Request::all_tests(sub {
    return shift->companies->scroll({email => 'test@test.com'});
});

subtest 'List companies' => sub {
    plan tests => 1;

    my $company_data = company_data();

    my $mock_ua = LWP::UserAgent->new();
    $mock_ua->map(qr#/companies/scroll$# => sub {
        my ($request) = @_;
    ...
    my $companylist = $client->companies->scroll();
    SharedTests::Company::test_resource_generation($companylist->companies->[0], $company_data->{companies}[0]);
};
...
subtest 'pagination' => sub {
    ...
    $mock_ua->map(qr#/companies/scroll\?scroll_param=TestScrollParam$# => sub {
        my ($request) = @_;
        my $response = HTTP::Response->new(
            '200',
            'OK',
    ...
    my $companylist = $client->companies->scroll();
    do {
        SharedTests::Company::test_resource_generation($companylist->companies->[0], shift(@data)->{companies}[0]);
    } while ($companylist = $companylist->pages->get_next());
};
