...
subtest 'List' => sub {
    ...
    my $resource = $client->companies->list();
...
subtest 'pagination' => sub {
    ...
    $mock_ua->map(qr#/companies\?per_page=1&page=2$# => sub {
        my ($request) = @_;
        my $response = HTTP::Response->new(
            '200',
            'OK',
    ...
    my $userlist = $client->companies->list();
    do {
...
sub company_data {
	return {
        ...
        pages => {
            type => 'page',
            next => 'https://api.intercom.io/companies?per_page=1&page=2',
            total_pages => 2,
        },
        companies => [{
            type                => 'company',
