...
subtest 'via ID' => sub {
    ...
    my $model = $client->users->get({id => 1});
...
subtest 'via Email' => sub {
    ...
    test_model_generation($client->users->get({email => 'test@test.com'}), $user_data);
};
...
subtest 'via UserID' => sub {
    ...
    test_model_generation($client->users->get({user_id => 23134}), $user_data);
};
